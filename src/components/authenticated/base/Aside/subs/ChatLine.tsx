...
const ChatLine = ({ chatline }: { chatline: ChatListDataType }) => {
  ...
  const last_message = chatline?.last_message.length > 30
  ? `${chatline?.last_message.substring(0, 30)}...`
  : chatline?.last_message;

  const lastMessage = chatline?.sender_id === chatline?.user_id ? 'You: ' + last_message: last_message;
  ...
  return (
    <div
      ...
      <div className="chat-time w-[20%] flex flex-col gap-1 justify-center">
        <div className="text-sm">{lastMessageTime}</div>
        <div className="text-xs">{isRead}</div>
      </div>
